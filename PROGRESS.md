...
- Implemented the untrained `v3.9_gain_invariant_mix` setup. It removes
  `osc_total_level` from the target vector, reconstructs a canonical total level of `1.0` from
  predicted balance, preserves v3.6 loss behavior for the remaining targets, and adds
  gain-invariant diagnostics. A render-equivalence test proves proportionally scaled oscillator
  pairs remain exactly equivalent after the current renderer normalization.
- Completed the next-generation strategy audit. The current renderer peak-normalization and
  ...
  capacity or probabilistic heads. Proposed, but did not initially implement, `v3.9_gain_invariant_mix`.
...
- Added paired product-benchmark calibration reporting for oscillator total level, detuned
  ...
  `0.0838`; the next controlled experiment is a loss-only bounded detuned-noise calibration
  ablation, not a waveform-target change.
