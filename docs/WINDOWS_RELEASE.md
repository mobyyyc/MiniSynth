...
- Selected model checkpoint under `resources/models/` when
  `models/v3.6_noise_detune_ablation.pt` exists at build time.
...
the Sandbox and run `NeuroWave Web Setup 0.1.1.exe` with
`neurowave-0.1.1-x64.nsis.7z` beside it. The bootstrapper validates that payload before
...
- `dist\cpu\win-unpacked\resources\models\v3.6_noise_detune_ablation.pt` exists.
